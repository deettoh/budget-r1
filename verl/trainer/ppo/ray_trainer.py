...
def compute_data_metrics(batch, use_critic=True):
    ...
    if "declared_budget" in batch.batch.keys():
        declared_budget = batch.batch["declared_budget"].detach().float()
        valid_declared = declared_budget >= 0
        if valid_declared.any():
            metrics["env/declared_budget/mean"] = (
                declared_budget[valid_declared].mean().item()
            )
        metrics["env/declared_budget/valid_ratio"] = (
            valid_declared.float().mean().item()
        )
    if "blocked_search_count" in batch.batch.keys():
        metrics["env/blocked_search/mean"] = (
            batch.batch["blocked_search_count"].detach().float().mean().item()
        )
...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        ...
        gen_config = GenerationConfig(
            ...
            retriever_name=self.config.retriever.name,
            index_path=self.config.retriever.index_path,
            corpus_path=self.config.retriever.corpus_path,
        ...
        if per_sample:
            ...
            for entry in per_sample:
                ...
                for key in ("all", source):
                    ...
                    ttc = entry["generated_tokens"] + entry["retrieved_tokens"]
                    bucket["ttc"].append(ttc)
                    bucket["generated_tokens"].append(entry["generated_tokens"])
                    bucket["retrieved_tokens"].append(entry["retrieved_tokens"])
                    bucket["has_answer"].append(1.0 if entry["has_answer"] else 0.0)
                    if entry.get("declared_budget", -1) >= 0:
                        bucket["declared_budget"].append(entry["declared_budget"])

            for source, bucket in buckets.items():
                mean_em = float(np.mean(bucket["em"]))
                mean_f1 = float(np.mean(bucket["f1"]))
                mean_mrc = float(np.mean(bucket["mrc"]))
                mean_ttc = float(np.mean(bucket["ttc"]))
                metric_dict[f"val/em/{source}"] = mean_em
                metric_dict[f"val/f1/{source}"] = mean_f1
                metric_dict[f"val/mrc/{source}"] = mean_mrc
                metric_dict[f"val/ttc/{source}"] = mean_ttc
                metric_dict[f"val/ces/{source}"] = cost_efficiency_score(
                    mean_f1, mean_ttc
                )
                metric_dict[f"val/generated_tokens/{source}"] = float(
                    np.mean(bucket["generated_tokens"])
                )
                metric_dict[f"val/retrieved_tokens/{source}"] = float(
                    np.mean(bucket["retrieved_tokens"])
                )
    ...
    def _maybe_save_best_checkpoint(self, val_metrics: dict) -> None:
        ...
        Called from fit() after _validate. Needs a real step saved.
        """
        ...
        marker_path = os.path.join(
            self.config.trainer.default_local_dir, "best_metric.json"
        )
        previous = None
        if os.path.exists(marker_path):
            try:
                with open(marker_path) as fh:
                    previous = json.load(fh).get("value")
            except (OSError, ValueError, json.JSONDecodeError):
                previous = None

        if previous is not None and current <= float(previous):
        ...
        if not os.path.isdir(step_path):
            # force a checkpoint so val "best" points at a real one
            self._save_checkpoint()
            if not os.path.isdir(step_path):
                return

        best_path = os.path.join(self.config.trainer.default_local_dir, "actor", "best")
        if os.path.isdir(best_path):
            shutil.rmtree(best_path)
        shutil.copytree(step_path, best_path)
        ...
        print(f"[trainer] new best by {key}: {current:.6f} (step {self.global_steps})")
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    if val_metrics:
                        with _timer("save_best", timing_raw):
